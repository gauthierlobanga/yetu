    ...
    return (
        ...
        >
            ...
            <div className="hidden gap-0.5 md:flex">
                <kbd
                    className={`grid h-5 min-w-5 place-items-center rounded bg-muted text-xs text-muted-foreground transition-all duration-200 ${
                        isModifierPressed
...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, children }) => {
    ...
    return createPortal(
        ...
        >
            <div
                className="h-full w-full max-w-full animate-in overflow-hidden bg-background shadow-2xl fade-in-0 zoom-in-95 md:h-auto md:max-h-[80vh] md:w-[90%] md:max-w-180 md:rounded-xl"
...
const HitsList = memo(function HitsList({
    ...
    if (!attributes || !mapping.primaryText) {
        throw new Error(
            'At least a primaryText is required to display results',
        );
    }
            ...
            {hits.map((hit: any, idx: number) => {
                ...
                return (
                    <a
                        ...
                        href={url ?? '#'}
                        target={
                            openResultsInNewTab && url ? '_blank' : undefined
                        }
                        rel={
                            openResultsInNewTab && url
                                ? 'noopener noreferrer'
                                : undefined
                        }
                        className="text-decoration-none flex animate-in cursor-pointer flex-row items-center gap-4 rounded-sm bg-background p-4 text-foreground fade-in-0 zoom-in-95 aria-selected:bg-blue-50 dark:aria-selected:bg-slate-900"
                        role="option"
                        aria-selected={isSel}
                        onClick={() => {
                    ...
                    >
                        {hasImage ? (
                            <div className="h-25 w-25 flex-[0_0_100px] items-center justify-center self-start overflow-hidden rounded-sm bg-muted">
                                {!isImageFailed ? (
                                    <img
...
const SearchInput = memo(function SearchInput(props: SearchInputProps) {
    ...
    return (
        ...
        >
            ...
            <input
                ref={props.inputRef}
                className="peer border-nonetext-foreground peer [::-webkit-search-results-button]:appearance-none[::-webkit-search-results-decoration]:appearance-none w-[90%] bg-transparent text-xl font-light outline-none [::-webkit-search-cancel-button]:appearance-none [::-webkit-search-decoration]:appearance-none"
                autoComplete="off"
                autoCorrect="off"
                autoCapitalize="off"
        ...
        <>
            ...
            <div className="flex flex-col">
                <SearchBox
                    query={query}
                    placeholder={
                        config.placeholder || 'What are you looking for?'
                    }
