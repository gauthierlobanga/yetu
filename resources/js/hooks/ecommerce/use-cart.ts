...
export function useCartItems() {
    ...
    const itemCount =
        cart?.items?.reduce((sum, item) => sum + item.quantite, 0) ?? 0;

    const updateQuantity = useCallback((itemId: number, quantity: number) => {
        router.patch(
            route('tenant.cart.update', itemId),
            { quantite: quantity },
            {
                preserveScroll: true,
                preserveState: true,
    ...
    const addToCart = useCallback(
        (productId: number | string, quantity = 1, variantId?: number) => {
            router.post(
                route('tenant.cart.add', productId),
                { quantity, variante_id: variantId },
                {
                    preserveScroll: true,
                    preserveState: true,
                    only: ['cart'],
    ...
    const applyCoupon = useCallback((code: string) => {
        router.post(
            route('tenant.cart.apply-coupon'),
            { code },
            {
                preserveScroll: true,
                preserveState: true,
