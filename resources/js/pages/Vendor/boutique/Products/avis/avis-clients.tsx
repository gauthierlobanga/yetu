...
// // Dans avis-clients.tsx
// interface Review {
//     id: number | string; // ← accepter string (UUID) ou number
//     note: number;
//     commentaire: string;
//     client: string;
//     date: string;
...
export function ReviewsSection({ productId, avis, ratingStats }: Props) {
    ...
    const ratingLabel = useMemo(() => {
        ...
        return labels[reviewRating] || 'Excellent';
    }, [reviewRating]);
    ...
    const handleSubmitReview = () => {
        ...
        router.post(
            route('tenant.products.reviews.store', productId),
            { note: reviewRating, commentaire: reviewComment.trim() },
            {
                preserveScroll: true,
                preserveState: true,
                showProgress: false,
    ...
    const canSubmit = reviewComment.trim().length >= 10 && !submitting;
    const isMinLengthReached = reviewComment.trim().length >= 10;
                    ...
                    {avis.length === 0 ? (
                        <div className="relative overflow-hidden rounded-3xl border border-dashed border-emerald-200 bg-linear-to-br from-emerald-50 to-white p-12 text-center shadow-sm dark:border-emerald-800/40 dark:from-emerald-950/20 dark:to-slate-900">
                            ...
                            <Button
                                ...
                                className="mt-8 rounded-full px-6"
                            >
                                <Star className="mr-2 h-4 w-4" /> Donner mon
                                avis
                            </Button>
                        </div>
                    ) : (
                        ...
                        <div className="space-y-6">
                                ...
                                <div className="flex gap-2">
                                    {[1, 2, 3, 4, 5].map((star) => (
                                        <button
                                            ...
                                            className="rounded-lg transition-transform hover:scale-110 focus:ring-2 focus:ring-emerald-500 focus:outline-none"
                                        >
                                            <Star
                                                className={cn(
                                                    'h-8 w-8',
                            ...
                            <div>
                                ...
                                <Textarea
                                    ...
                                    placeholder="Partagez votre expérience avec ce produit..."
                                    className={cn(
                                        'resize-none rounded-2xl border-slate-200 bg-slate-50/70 transition-all dark:border-slate-700 dark:bg-slate-800/50',
                                        isMinLengthReached &&
...
function StatCard({
    ...
    return (
        <motion.div
            ...
            className="rounded-2xl border border-slate-200/70 bg-white/80 px-4 py-3 shadow-sm backdrop-blur-xl transition-all hover:shadow-md dark:border-slate-800/60 dark:bg-slate-900/70"
        >
            <div className="mb-2 flex items-center gap-2 text-emerald-600 dark:text-emerald-400">
                {icon}
            </div>
