...
namespace App\Http\Controllers\Tenants\dashbords\admin;
...
class DashboardTenantController extends Controller
{
    public function index(Request $request)
    {
        if (! $this->hasDashboardSchema()) {
            return Inertia::render('tenants/dashboard/admins/dashboard', $this->emptyDashboardPayload($request));
        }
        ...
        $categoriesList = PostCategory::orderByDesc('nom')->get(['id', 'nom', 'slug']);

        return Inertia::render('tenants/dashboard/admins/dashboard', [
            'posts' => $posts,
            'stats' => $stats,
