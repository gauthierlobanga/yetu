...
class CategoryTenantController extends Controller
{
    public function index()
    {
        ...
        return Inertia::render('tenants/Shop/Categories/Index', ['categories' => $categories]);
    }
    ...
    public function show(ProductCategory $category)
    {
        ...
        $products = $category->products()->published()->inStock()->paginate(24)
            ->through(fn ($p) => app(ProductTenantController::class)->formatProduct($p));

        $subcategories = $category->children()->active()->ordered()->get()->map(fn ($c) => $this->formatCategory($c));
        $breadcrumb = $category->getBreadcrumb();

        return Inertia::render('tenants/Shop/Categories/Show', [
    ...
    private function formatCategory(ProductCategory $category): array
    {
        return [
            ...
            'url' => route('tenant.categories.show', $category->slug),
