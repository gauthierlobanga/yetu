...
class NewsletterCampaignController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($campaign->status === 'programme' || $campaign->status === 'envoye') {
            ...
            if (class_exists(\App\Jobs\ProcessNewsletterCampaign::class)) {
                $delay = $campaign->scheduled_at ? \Carbon\Carbon::parse($campaign->scheduled_at) : now();
                \App\Jobs\ProcessNewsletterCampaign::dispatch($campaign)->delay($delay);
            }
        }
    ...
    public function sendTest(Request $request, $id)
    {
        ...
        $send->id = \Illuminate\Support\Str::uuid();
        $send->campaign_id = $campaign->id;

        try {
            \Illuminate\Support\Facades\Mail::to($request->email)
                ->send(new \App\Mail\CampaignMailable($campaign, $send));
